...
st.plotly_chart(px.scatter(shot_data, x='h_shots', y='fth_goals',
                           title='Home Shots vs Home Goals', trendline='ols'))
st.plotly_chart(px.scatter(shot_data, x='a_shots', y='fta_goals',
                           title='Away Shots vs Away Goals', trendline='ols'))
st.plotly_chart(px.scatter(shot_data, x='total_shots', y='total_goals',
                           title='Total Shots vs Total Goals', trendline='ols'))
...
arsenal['fouls_committed'] = arsenal.apply(
    lambda r: r['h_fouls'] if r['hometeam'] == 'Arsenal' else r['a_fouls'], axis=1)
arsenal['fouls_suffered'] = arsenal.apply(
    lambda r: r['a_fouls'] if r['hometeam'] == 'Arsenal' else r['h_fouls'], axis=1)

fouls_df = pd.DataFrame({
    'Type': ['Fouls Committed', 'Fouls Suffered'],
    'Average': [arsenal['fouls_committed'].mean(), arsenal['fouls_suffered'].mean()]
})
st.plotly_chart(px.bar(fouls_df, x='Type', y='Average', color='Type',
                       title="Arsenal: Average Fouls"))
...
coote_liv['liv_yellow'] = coote_liv.apply(
    lambda r: r['h_yellow'] if r['hometeam'] == 'Liverpool' else r['a_yellow'], axis=1)
coote_liv['opp_yellow'] = coote_liv.apply(
    lambda r: r['a_yellow'] if r['hometeam'] == 'Liverpool' else r['h_yellow'], axis=1)
coote_liv['liv_fouls'] = coote_liv.apply(
    lambda r: r['h_fouls'] if r['hometeam'] == 'Liverpool' else r['a_fouls'], axis=1)
coote_liv['opp_fouls'] = coote_liv.apply(
    lambda r: r['a_fouls'] if r['hometeam'] == 'Liverpool' else r['h_fouls'], axis=1)
...
st.plotly_chart(px.pie(pie_data, names='Result', values='Count',
                       title="Arsenal vs Big Six"))
